...
import { User, Goal, WeeklyUpdate, Proof, WorkoutDay, AdminSettings } from './types';
...
function AppContent() {
  ...
  const recalculateUserConsistency = () => {
    setUsers(currentUsers => {
      ...
      const goalData = goals.map(g => ({ userId: g.userId, isCompleted: g.isCompleted }));
      
      const updatedUsers = updateAllUsersConsistency(currentUsers, workoutDays, goalData, currentWeek);
      
      // Log changes and update database
      updatedUsers.forEach(updatedUser => {
        const originalUser = currentUsers.find((u: User) => u.id === updatedUser.id);
        if (originalUser && (
          originalUser.cleanWeeks !== updatedUser.cleanWeeks ||
          originalUser.missedWeeks !== updatedUser.missedWeeks ||
          originalUser.currentConsistencyLevel !== updatedUser.currentConsistencyLevel ||
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      try {
        ...
        setTimeout(() => {
          const currentWeek = getCurrentWeek();
          const goalData = dbGoals.map(g => ({ userId: g.userId, isCompleted: g.isCompleted }));
          const updatedUsers = updateAllUsersConsistency(dbUsers, dbWorkouts, goalData, currentWeek);
  ...
  const updateGoal = async (updatedGoal: Goal) => {
    try {
      ...
      const savedGoal = await apiService.updateGoal(updatedGoal.id, updatedGoal);
      
      // Update local state
      setGoals(prevGoals => {
        const newGoals = prevGoals.map(g => g.id === updatedGoal.id ? savedGoal : g);
        
        // Trigger consistency recalculation if goal completion status changed
        if (updatedGoal.isCompleted !== prevGoals.find(g => g.id === updatedGoal.id)?.isCompleted) {
  ...
  const deleteUser = async (userId: string) => {
    try {
      ...
      // Update local state
      setUsers((prevUsers: User[]) => prevUsers.filter((u: User) => u.id !== userId));
      setGoals((prevGoals: Goal[]) => prevGoals.filter((g: Goal) => g.userId !== userId));
      setWeeklyUpdates((prevUpdates: WeeklyUpdate[]) => prevUpdates.filter((w: WeeklyUpdate) => w.userId !== userId));
      setProofs((prevProofs: Proof[]) => prevProofs.filter((p: Proof) => p.userId !== userId));
      setWorkoutDays((prevWorkouts: WorkoutDay[]) => prevWorkouts.filter((w: WorkoutDay) => w.userId !== userId));
